...
TEST(OctaveTransposerPropertyTest, SequenceMatchesClampedModel) {
    hegel::test([](hegel::TestCase& tc) {
        const auto operations = tc.draw(
            "operations",
            gs::vectors(gs::booleans())
        );
        const auto type = keyMessageType(tc.draw(
            "type_index",
        ...
        ASSERT_EQ(transposer.transpose(message).key, expected_key)
            << "octave transposition disagrees with model";
    });
}
...
TEST(OctaveTransposerPropertyTest, PreservesNonKeyFields) {
    hegel::test([](hegel::TestCase& tc) {
        ...
        ASSERT_TRUE(equalExceptKey(transposer.transpose(message), message))
            << "octave transposition changed a non-key field";
    });
}
...
TEST(OctaveTransposerPropertyTest, PreservesNonKeyMessages) {
    hegel::test([](hegel::TestCase& tc) {
        ...
        ASSERT_TRUE(equalMessage(transposer.transpose(message), message))
            << "octave transposition changed a non-key message";
    });
}
